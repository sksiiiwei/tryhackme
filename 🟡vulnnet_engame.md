...
решила пофаззать сабдомены и - бинго! нам выдало несколько. но на админ1 и апи, к сожалению, ничего интересного не нашла (пробовала пофаззить, admin1 выдает достаточно много всего, но в основном мусорного. из того что +- может пригодиться - страница логина на ```http://admin1.vulnnet.thm/typo3```. запомним)
...
проверила его, и да, действительно, он не защищен и мы можем спокойно на него попасть

<img width="700" height="190" alt="image" src="https://github.com/user-attachments/assets/cf67a683-1a9b-4559-b10f-bf130a231efb" />

попробовала проверить на уязвимость к sqli, и параметр blog оказался уязвим. теперь посмотрим, какие есть базы данных
...
проверила все таблицы на всякий случай, но самое интересное нашла только в дампе таблицы users (```-D blog -T users --dump```). в ней оказались пароли всех пользователей блога (у меня начало вывода обрезалось, решается флагом ```--start 1```). окей, но форму логина я в блоге не видела, да и обычные пользователи нас особо не интересуют (но то что пароли хранятся незахешированными - вкусно, хотя и малореалистично), но запомнили
...
+---------------------------------------------+
```
cудя по специфическим названиям таблиц (be_, fe_, sys_) - это и есть бд от cms (TYPO3). окей, попробуем сдампить самое интересное - логины/хэши паролей админов (```-D vn_admin -T be_users --dump```)

<img width="700" height="51" alt="image" src="https://github.com/user-attachments/assets/efb59e17-e323-4afb-a6fb-4675c20409aa" />

и вот оно - хеш админского пароля. но радоваться пока нечему - захеширован пароль алгоритмом $argon2i$, а параметры (m=65536 — 64 МБ памяти, t=16 — 16 итераций) делают его очень неприятным. ломать его долнго и больно (я на стадии отрицания закинула на минут пятнадцать в john'а с rockyou, надеясь, что пароль может быть где-то в начале словаря, но неа. так что таким образом пароль не достать). полазив еще по таблицам и не найдя ничего толкового, подумала, что в теории, паролем админа может оказаться один из паролей в таблице users (из блога). юзернейма админа (chris_w) там нет, но один и тот же пароль он вполне мог использовать дважды. составила файл с дампом паролей (вообще, по-хорошему для этой задачи можно было бы написать скрипт на питоне, но я поленилась и просто попросила нейросетку выписать все пароли) и закинула в джона

```
john --wordlist=pass.txt hash.txt
```

и, ура, мы действительно смогли таким образом расхешировать пароль. далее перешла на ранее найденную страницу ```http://admin1.vulnnet.thm/typo3``` и залогинилась по полученным кредам. теперь мы chris_w! остается получить реверсшелл и попасть на сервер. самое логичное - подгрузить php файл в filelist, но расширение php загружать запрещено. но в настройках (system tools -> settings) я увидела configure installiation-wide options - бинго. 

<img width="700" height="403" alt="image" src="https://github.com/user-attachments/assets/180ad0f3-03ab-4f17-9a7f-accfd3e9afc9" />

удаляем запрет на подгрузку php в параметре ```[BE][fileDenyPattern]```и спокойно подгружаем скрипт (я сгенерировала его через msfvenom, чтобы поймать шелл в msfconsole)

<img width="700" height="311" alt="image" src="https://github.com/user-attachments/assets/0560a4d5-8805-42be-b4da-15b3c0d68527" />

переходим по адресу ```http://admin1.vulnnet.thm/fileadmin/user_upload/rceraw.php```, чтобы запустить скриптЮ и вот, теперь мы внутри с правами юзера www-data

### Взятие юзера

видим, что user.txt взять не можем, он лежит у пользователя system. походу, сначала придется брать его. подгрузила на сервер линпис (можно через метерпретер в мсфконсоли, можно просто поднять питоновский сервер). 
...
```
увидев 631 порт, сразу почему-то решила, что пользователь system будет состоять в группе lpadmin(и, конечно, ниче не проверила. видимо, простые машины уже привили привычку, что если есть что-то подозрительное - это сработает. отстой), и этот вектор атаки понадобится в будущем для повышения привилегий до рута через веб-интерфейс для администрирования принтеров, ака cups. подгрузила статически скомпилированный сокат на машину, дала права на исполнение и пробросила порт (```./socat TCP-LISTEN:9999,fork,reuseaddr TCP:127.0.0.1:631```). теперь можно зайти в интерфейс из браузера (```http://ip:9999```). попала на страничку bad request из-за защиты от DNS rebinding. через бурп поменила хоста на localhost и наконец оказалась внутри интерфейса. перешла во вкладку administration, чтобы отредактировать cupsd.conf и добавить в него ```FileDevice Yes``` (чтобы можно было создавать принтеры, которые сохраняют печать в локальные файлы), надеясь провести атаку вроде:

```
lpadmin -p pwn_printer -E -v file:/etc/cron.d/root_shell -m raw # создаем принтер, который записывает сырые данные(без шрифтов и тп) в кронтаб, сразу включаем его
echo "* * * * * root chmod +s /bin/bash" | lp -d pwn_printer
```
там у меня запросили креды, и я, уверенная, что подготовилась к взятию рута и креды от system подойдут, пошла искать способ повыситься до system. уже потом до меня доперло, что у system нужной группы нет, да и креды system не подошли, так что файл так и остался неотредактированным. а теперь к более успешным решениям. в домашней папке system можно заметить интересную скрытую директорию - в ней находится кэш мозиллы, где можно, в теории, найти логины и пароли (это работает в основном в мозилле и в старых версиях хрома)
...
заходим в profiles.ini и видим - один из профилей них действительно не инициализирован (это случается либо в случае если папка профиля пустая и firefox не стал ее инициализировать, либо кто-то вручную подчистил файл. но тк она не пустая, то автор походу просто очень пытается усложнить нам жизнь)
...
снова запускаем firefox_decrypt, выбираем нужный профиль и на этот раз действительно находим логин/пароль (от платформы thm, кстати). попробовала переключиться на system с найденным паролем. сработало! (спасибо реюзу)
...
```
а конкретно openssl ```=ep``` (знак ```=ep``` без указания конкретного имени означает, что программе даны вообще все возможные права (аналог root)). следовательно, ам нужно использовать внутренние функции openssl, которые заставят программу сделать то, что нам надо. пишем exploit.c
...
```
(логика составления кода исходит из ожиданий openssl, чтобы ничего не поломалось. типа: если пользователь хочет загрузить динамический движок (engine), я должен загрузить файл и найти в нем функцию с конкретным именем bind_engine. и нам не нужно использовать что-то вроде ```__attribute__((constructor))```, чтобы сразу вызвать функцию, потому что openssl c флагом -engine сразу вызывает ее для ее инициализации)
```
// примерно так это выглядит после работы макроса (для понимания, почему мы передаем аргументы, которые позже не используем):
